...
const Cell = ({ isAlive, onClick, numResizes }) => {
  ...
  useEffect(() => {
    ...
    const updateCellSize = () => {
      ...
      const cellsPerRow = Math.floor(screenWidth / 30); // 20px is the minimum width for a cell
      const cellsPerColumn = Math.floor(screenHeight / 30); // 20px is the minimum height for a cell
