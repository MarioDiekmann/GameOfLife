        ...
        setGrid(prevGrid => {
            ...
            const newGrid = prevGrid.map((row, rowIndex) =>
                row.map((cell, colIndex) => {
                    ...
                    // Determine the next state of the cell based on neighbors
                    const nextState = cell ? gridCount === 2 || gridCount === 3 : gridCount === 3;
                    
                    // If any cell changes its state, we do not have a stillstand
                    if (nextState !== cell) {
                        stillstand = false;
                    }
            ...
            if (stillstand) {
                stillstandDetected();  // Trigger stillstand detection
            }
