...
const Game = () => {
    ...
    const expandGrid = useCallback((indizesAlive) => {
        ...
        for (let row = 0; row < size; row++) {  
            for (let col = 0; col < size; col++) {  
                newGrid[row + size][col + size] = grid[row][col]; // Keep old grid cells in the new grid
            }
        }
    
        // Set alive cells in the new grid based on the indices alive
        for (const [row, col] of indizesAlive) {
    ...
    }, [isEvolving, checkEdges, expandGrid, updateGrid]);
