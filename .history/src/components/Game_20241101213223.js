...
const Game = () => {
    ...
    return (
        <div className="game-container">
            <NavBar closeForm={closeForm} />
            <h1 className="game-title">Game of Life</h1>
            <div className="grid-container">
                {renderGrid()}
            </div>
            <div className="button-container">
                <button onClick={startEvolution}>
                    {isEvolving ? "Pause" : "Start"}
                </button>
                <button onClick={resetGrid}>Reset</button>
                <button onClick={() => setPatternRequested(true)}>Load Pattern</button>
                <button onClick={() => setRegistrationRequested(true)}>Register</button>
                <button onClick={() => setLoginRequested(true)}>Login</button>
            </div>
            {patternRequested && <LoadPatternForm closeForm={closeForm} setGrid={setGrid} />}
            {registrationRequested && <RegistrationForm closeForm={closeForm} />}
            {loginRequested && <LoginForm closeForm={closeForm} />}
            <div className="epoch-counter">Epoch: {epochs}</div>
        </div>
    );
};
