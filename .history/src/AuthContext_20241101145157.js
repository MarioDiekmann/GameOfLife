...
export const AuthProvider = ({ children }) => {
  ...
  const handleLogin = (newToken) => {
    ...
    // Manually check for expiration immediately
  ...
  const handleLogout = () => {
    ...
    setSessionExpired(false);
    clearInterval(expirationCheckInterval.current);  // Clear the interval on logout
  };
  ...
  useEffect(() => {
    if (token) {
      ...
      // Set a new interval for expiration checking
